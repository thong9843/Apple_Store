...
const CheckoutFromCartItems = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { userId, cartItemIds } = location.state || {};
  ...
  const [discountedProducts, setDiscountedProducts] = useState(new Map());
  ...
  useEffect(() => {
    const fetchCartAndAddresses = async () => {
      try {
        ...
        const cartItemsData = cartRes.data.items.filter((item) =>
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8080/api/orders/from-cart-items`,
        null,
        {
          params: {
            userId,
    ...
    try {
      ...
      for (const item of cartItems) {
        try {
          ...
          console.log(
            `Product variant ${item.productVariantId} cannot apply discount code`
          );
        }
      }
      ...
      if (hasValidDiscount) {
        ...
        alert(
          `Discount code applied successfully ${newDiscountedProducts.size} products!`
        );
      } else {
...
export default CheckoutFromCartItems;
